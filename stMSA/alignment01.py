...
def get_transform(adata_list, dst_id, src_id_list, target_label_id, threshold=50, max_iterations=5000, tolerance=1e-3,
                  seed=0):
    transform_matrix = {}
    ...
    # 陷入局部最优 -> 尝试多次不同旋转和平移来跳出局部最优
            if threshold < mean_error:
                ...
                for _ in range(num_attempts):
                    # 随机生成旋转角度
                    rotate_deg = np.random.uniform(0, 2 * np.pi)
                    ...
                    translation_vector[2] = 0  # 确保 z 方向平移为 0
                    translation_matrix = np.eye(3)
                    translation_matrix[:2, 2] = translation_vector[:2]

                    # 先旋转再平移
                    combined_matrix = np.dot(translation_matrix, rotation_matrix)
                    new_src = coor_transform(cur_src[:2, :].T, combined_matrix)
                    ...
                    if new_mean_error < best_error:
                        ...
                        best_transform = combined_matrix
